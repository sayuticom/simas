...
<div class="bg-white rounded-lg shadow p-6">
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-500">Perbarui identitas, reset password, atau ubah hak akses masjid.</p>
    </div>
    ...
    <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}" required class="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            <div>
                <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                <input id="email" name="email" type="email" value="{{ old('email', $user->email) }}" required class="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            <div class="md:col-span-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
                <label class="inline-flex items-center gap-3 text-sm font-semibold text-gray-800">
                    <input id="reset_password" name="reset_password" type="checkbox" value="1" {{ old('reset_password') ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    Ubah Password
                </label>
                <p class="mt-2 text-xs text-gray-500">Centang hanya jika password user perlu direset.</p>
            </div>
            <div id="password-fields" class="contents">
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700">Password Baru</label>
                    <input id="password" name="password" type="password" minlength="8" autocomplete="new-password" class="password-input mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-700">Konfirmasi Password Baru</label>
                    <input id="password_confirmation" name="password_confirmation" type="password" minlength="8" autocomplete="new-password" class="password-input mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500">
                </div>
            </div>
        </div>
        ...
        @if($isSuperSuperuser)
            <div class="rounded-lg border border-indigo-200 bg-indigo-50 p-4">
                <p class="text-sm font-semibold text-indigo-800">Super Superuser</p>
                <p class="mt-2 text-xs text-indigo-700">Role otoritas tertinggi dipertahankan dan tidak dapat diubah dari form ini.</p>
            </div>
        @elseif($canAssignSuperuser)
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                <label class="inline-flex items-center gap-3 text-sm font-semibold text-gray-800">
                    <input id="is_superuser" name="is_superuser" type="checkbox" value="1" {{ old('is_superuser', $isSuperuser) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    Jadikan user sebagai Superuser
                </label>
                <p class="mt-2 text-xs text-gray-500">Superuser memperoleh akses global dan tidak menggunakan akses masjid.</p>
            </div>
        @endif
        ...
        <div id="access-section" class="rounded-lg border border-gray-200 p-5">
            ...
            <div id="access-rows" class="space-y-3">
                @php
                    $accessRows = old('accesses', $assignedAccesses ?: [['mosque_id' => '', 'role_id' => '']]);
                @endphp
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="rounded-lg bg-indigo-600 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">Simpan Perubahan</button>
        </div>
    </form>
</div>
...
<script>
    (() => {
        ...
        const passwordInputs = passwordFields.querySelectorAll('.password-input');
        const preserveGlobalAccess = @json($isSuperSuperuser);
        ...
        const updateAccessState = () => {
            const disabled = preserveGlobalAccess || (superuser ? superuser.checked : false);
        ...
        const updatePasswordState = () => {
            const enabled = resetPassword.checked;
            passwordFields.classList.toggle('opacity-50', !enabled);
        ...
        resetPassword.addEventListener('change', updatePasswordState);
        updateNames();
        updateAccessState();
        updatePasswordState();
    })();
