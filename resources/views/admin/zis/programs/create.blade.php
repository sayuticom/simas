...
@section('title', 'Tambah Program ZIS')
@section('page_title', 'Tambah Program ZIS')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-sm">
        <form action="{{ route('zis.programs.store') }}" method="POST">
            @csrf
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" rows="4" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('deskripsi') }}</textarea>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">Target Dana (Rp)</label>
                    <input type="number" name="target_dana" value="{{ old('target_dana') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                </div>
                <div>
