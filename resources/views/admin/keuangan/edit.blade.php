...
<div class="bg-white rounded-lg shadow p-6">
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">{{ $transaction->type === 'masuk' ? 'Transaksi pemasukan lama ditampilkan sebagai readonly. Dana masuk baru dicatat melalui Modul ZIS.' : 'Perbarui informasi pengeluaran kas masjid umum.' }}</p>
    </div>
    ...
    <form action="{{ route('keuangan.transaksi.update', $transaction) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
                <div class="flex items-center justify-between gap-3">
                    ...
                    @unless($readonly ?? false)
                        <a href="{{ route('keuangan.kategori.create', ['return_to' => 'transaction_edit', 'transaction_id' => $transaction->id]) }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-800">
                            + Tambah Kategori
                        </a>
                    @endunless
                </div>
            ...
            <div>
                ...
                <select name="cash_account_id" class="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500" required @disabled($readonly ?? false)>
                    <option value="">Pilih akun kas</option>
                    @foreach($cashAccounts as $account)
                        <option value="{{ $account->id }}" {{ old('cash_account_id', $transaction->cash_account_id) == $account->id ? 'selected' : '' }}>
